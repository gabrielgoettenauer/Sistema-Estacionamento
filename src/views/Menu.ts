...
export class Menu {
    ...
    constructor(patio: PatioController, clienteCtrl: ClienteController) {
        this.patio = patio;
        ...
        while (true) {
            ...
            switch (op) {
                case "1":
                    ...
                    console.log(this.clienteCtrl.cadastrar(nome, doc));
                    break;

                case "2":
                    console.log("\n--- ENTRADA ---");
                    const cpf = await this.rl.question("CPF do Dono: ");
                    const cliente = this.clienteCtrl.buscarPorDocumento(cpf);
                    
                    const opcaoTipo = await this.rl.question("Tipo (1-Carro | 2-Moto): ");
                    ...
                    const marca = await this.rl.question("Modelo: ");
                    
                    console.log(this.patio.registrarEntrada(tipoEnum, placa, marca, cliente));
                    break;

                case "3":
                    const p = await this.rl.question("Placa para saída: ");
                    const h = parseInt(await this.rl.question("Horas: "));
                    const res = this.patio.registrarSaida(p, h);
                    if (res) {
                        res.veiculo.imprimirDados();
                        console.log(` TOTAL: R$ ${res.total},00`);
                    } else {
